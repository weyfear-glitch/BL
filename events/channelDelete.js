...
// Tracker : userId -> { channelDeletes: [], roleDeletes: [] }
...
async function handleNuke(guild, executor, type, client) {
  ...
  const embed = new EmbedBuilder()
    .setColor(0xe74c3c)
    .setTitle('💣 Anti-Nuke déclenché — Banni automatiquement')
    .addFields(
      { name: 'Exécutant', value: `${executor.tag} (${executor.id})`, inline: true },
      { name: 'Type d\'action', value: type, inline: true },
...
// Exporter séparément pour les suppressions de rôles
// Le fichier roleDelete.js réutilise le même tracker
